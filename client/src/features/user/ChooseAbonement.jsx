...
const ChooseAbonement = () => {
  const { abonement, user_id } = useSelector(selectUser)[0];
  ...
  const handleClick = async (id_abonement) => {
    try {
      await updateAbonement({ abonement: id_abonement, user_id }).unwrap();
    } catch (err) {
      console.log(err);
    }
  };
      ...
      <div className="max-w-[1240px] mx-auto py-12 ">
        ...
        <div className="grid md:grid-cols-2 ">
          {abonementsToChoose.map((item) => {
            const { description, name_of_abonement, price, id_abonement } =
              item;
            return (
              <div className="bg-white text-slate-900 m-4 p-8 rounded-xl shadow-2xl relative hover:scale-105 ease-in duration-300">
                ...
                <div className="text-2xl">
                  ...
                  <button
                    style={{
                      backgroundColor:
                        abonement === id_abonement ? "grey" : currentColor,
                    }}
